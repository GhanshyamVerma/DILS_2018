...
\title{\bf \bf Respiratory Viral Data Set: SVM on 64 Subjects Data at 0 and 48 Hours}
...
\section{SVM on 64 Infected Subjects Data at 0 and 48 Hours}
For this report, I have taken 64 infected subjects into consideration. I have extracted 2 time-specific data points for each infected and unambiguous subject of these challenge studies. For each infected subject, the first sample is the sample taken before inoculation (at 0 hour) i.e. having label “not infected” or “negative” and the second sample is the sample which is nearest to 48 hour i.e. having label “infected” or “positive”. \\
...
getwd()
setwd("/Users/ghanshyamverma/Documents/Respiratory_Data/Infected_64_Subjects_0_48_Hour/SVM")
@
...
\section{Tune Non-Linear SVM (with RBF Kernel) Parameters}
<<include=TRUE, message=FALSE, warning=FALSE, error=FALSE>>=
# Enable Parallel Processing
library(doSNOW)
library(doParallel)
